...
with open("/storage/emulated/0/Download/LOGOR_v1.0.1/gest_iacomino/grower_list.txt", 'r') as f:
    f.seek(0)
    prod_matrix = [[]]
    for x in range(R_LEN):
        ...
        for y in range(R_LEN):
            word = growerArray[x].prod.split(",")    
            prod_matrix.insert(y, word)
        print(prod_matrix)
        '''
...
def main():
    ...
    f = open("/storage/emulated/0/Download/LOGOR_v1.0.1/gest_iacomino/label_report.txt", 'a+')
    if os.stat("/storage/emulated/0/Download/LOGOR_v1.0.1/gest_iacomino/label_report.txt").st_size == 0:
        f.seek(0)
        f.write("0\n")    
    f.close()
    ...
    index = int()
    with open("/storage/emulated/0/Download/LOGOR_v1.0.1/gest_iacomino/label_report.txt", 'r') as f:
    ...
    pdf.add_page()
    pdf.image('/storage/emulated/0/Download/LOGOR_v1.0.1/gest_iacomino/bin/Logo-Starline-Flowers_alpha027.png', x = 10, y = 75, h = 140, w = 193)
    ...
    #  funziona provvisoria per gestione prodotto
    print(colored("\nSeleziona prodotti: \n" + growerArray[gIn].prod, 'red', 'on_green'))
    provv_prodinput = input(colored("Prodotto:\n>  ",'yellow'))
    pdf.cell(190, 20, provv_prodinput, ln=True, border='L,R,B', align='L')
    ...
    while (t_pl >= curr_pl):
        ...
        pdf.cell(190, 10, 'P' + str(curr_pl+1) + ' ' + str(qty) + ' ' + str(clr) + ' ' + str(measure)+" cm" + ' st.' + str(stem), border='L,R', align='L')
        pdf.cell(190, 5, '', ln=True, border='L,R')

        print(colored("\n\n==================================",'green'))
        print(colored("P"+str(curr_pl+1)+" > "+str(qty)+" "+str(clr)+" "+str(measure)+" cm"+" st."+str(stem),'green'))
        print(colored("==================================\n\n",'green'))
        ...
        if tmp_choice == "y":
            ...
            cart_plane += 1
        if (cart_plane >= 4):
            print("Pianali tot. -> " + str(t_pl+1) + "  |  " + "CC tot. -> " + str(t_cart) + "\n")
            time.sleep(0.25)
    ...
    if(f):
        print("** /storage/emulated/0/Download/LOGOR_v1.0.1/gest_iacomino/label_report.txt aggiornato con successo! **\n")
    #name with incremental enumeration
    l_name = "ORDINE_N0"+str(index)+".pdf"
    
    #write new line on report file
    with open('/storage/emulated/0/Download/LOGOR_v1.0.1/gest_iacomino/label_report.txt', 'a+') as f:
    ...
    pdf.output('/storage/emulated/0/Download/LOGOR_v1.0.1/gest_iacomino/RTP_labels/' + l_name)
    ...
    #creation of final .pdf file
    pdf.output('/storage/emulated/0/Download/LOGOR_v1.0.1/gest_iacomino/RTP_labels/logistic_labels/' + log_l_name)
